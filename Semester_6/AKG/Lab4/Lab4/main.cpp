...
int main()
{
	...
	std::string path{ "..\\Assets\\Shovel\\" };
	ObjParser parser{ path + "shovel.obj" };
	sf::Image diffuseMap;
	diffuseMap.loadFromFile(path + "diffuse.png");
	sf::Image specularMap;
	specularMap.loadFromFile(path + "specular.png");
	sf::Image normalMap;
	normalMap.loadFromFile(path + "normal.png");
			...
			threads[i] = std::thread([&, i]()
				{
					for (size_t j = 0; j < fragment.size(); j++) 
					{
						...
						std::vector<std::vector<float>>& faceTextures = fragment.at(j).getVT();
						...
						std::vector<float>& texture0 = faceTextures.at(0);
						std::vector<float>& texture1 = faceTextures.at(1);
						std::vector<float>& texture2 = faceTextures.at(2);

						glm::vec3 textureVec0{ texture0.at(0), texture0.at(1), texture0.at(2) };
						glm::vec3 textureVec1{ texture1.at(0), texture1.at(1), texture1.at(2) };
						glm::vec3 textureVec2{ texture2.at(0), texture2.at(1), texture2.at(2) };

						float w1 = calcVert0.w;
						float w2 = calcVert1.w;
						float w3 = calcVert2.w;
						...
						if (scalar > 0)
						{
							...
							std::vector<glm::vec3> textureVectors{ textureVec0, textureVec1, textureVec2 };
							...
							Algorithms::rasterTriangle(vertices, worldVertices, normalVectors, textureVectors, cameraPos, image, colorComponents,
								...
								diffuseMap, specularMap, normalMap, (specularMap.getSize().x != 0 && specularMap.getSize().y != 0), (normalMap.getSize().x != 0 && normalMap.getSize().y != 0));
						}
					}
				}
			);
