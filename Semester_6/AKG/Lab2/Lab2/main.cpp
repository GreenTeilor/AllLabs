			...
			threads[i] = std::thread([&, i]()
				{
					for (size_t j = 0; j < fragment.size(); j++) 
					{
						...
						std::vector<float>& vert0 = currentFace.at(0);
						std::vector<float>& vert1 = currentFace.at(1);
						std::vector<float>& vert2 = currentFace.at(2);
						...
						float lightIntensity = glm::dot(glm::normalize(normal), glm::normalize(reversePolygonCameraDirection));

						if (scalar > 0)
						{
							std::vector<glm::vec4> vertices{ calcVert0, calcVert1, calcVert2 };
							Algorithms::rasterTriangle(vertices, image, sf::Color{ (sf::Uint8)(255 * lightIntensity), (sf::Uint8)(255 * lightIntensity), (sf::Uint8)(255 * lightIntensity) },
								zBuffer, (calcVert0.z + calcVert1.z + calcVert2.z) * 0.33333);
						}
					}
				}
			);
