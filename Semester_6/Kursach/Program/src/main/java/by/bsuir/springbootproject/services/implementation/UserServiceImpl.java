...
import by.bsuir.springbootproject.exceptions.NoProductInInventoryException;
...
import by.bsuir.springbootproject.repositories.InventoryRepository;
...
public class UserServiceImpl implements UserService {
    ...
    public ModelAndView makeOrder(User user, Cart cart) throws InsufficientFundsException, NoProductsInOrderException,
            NoResourceFoundException, NoProductInInventoryException {
        ...
        if (cart.getProducts().stream().anyMatch(p -> inventoryRepository.findByProductId(p.getId()).
                orElseThrow(() -> new NullPointerException("Нет сведений о продукте в инвентаре")).getAmount() < 1)) {
            throw new NoProductInInventoryException("На складе не хватает книг для оформления заказа");
        }
        Order order = Order.builder().userId(user.getId()).date(LocalDate.now()).products(new ArrayList<>
                (cart.getProducts())).price(orderPrice).build();
        orderRepository.save(order);
        cart.getProducts().forEach(p -> inventoryRepository.findByProductId(p.getId()).ifPresent(i -> {
