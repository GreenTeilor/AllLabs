...
Termin::Termin(const std::string& name, std::vector<int> pagesNums) : m_parentTermin{ nullptr }, m_name{ name }, m_pagesNums{ pagesNums }
{
...
{
	std::cout << "Имя термина: " << m_name << ". Термин расположен на страницах: ";
	this->printPages();
	std::cout << std::endl;
}
...
void Termin::sortByPages_private(Termin& termin)
{
	if (termin.m_subTermins.size() == 0)
		return;
	for (int i = 0; i < termin.m_subTermins.size(); ++i)
		this->sortInts(m_subTermins[i].m_pagesNums);
	for (int i = 1; i < termin.m_subTermins.size(); ++i)
		for (int j = 0; j < termin.m_subTermins.size() - i; ++j)
			if (termin.m_subTermins[j].m_pagesNums[0] > termin.m_subTermins[j + 1].m_pagesNums[0])
			{
				std::vector<int> temp{termin.m_subTermins[j].m_pagesNums};
				termin.m_subTermins[j].m_pagesNums = termin.m_subTermins[j + 1].m_pagesNums;
				termin.m_subTermins[j + 1].m_pagesNums = temp;
			}
	for (int i = 0; i < termin.m_subTermins.size(); ++i)
