...
namespace form {
	...
	public:
		...
		{
			None.loadFromFile("../fonts/Samson.ttf");			//передаем нашему шрифту файл шрифта
	...
	public:
		Input(double _x = 0, double _y = 0, double _width = 150, double _height = 30, string _text = "")
		{
			...
			TXT.setFont(None);							// загружаем фрифт
			TXT.setCharacterSize(40); 					// в пикселях, а не точках!
		...
		bool select(Vector2i _mouse, RenderWindow& window)
		{
			if (FloatRect(window.getSize().x / 10, window.getSize().y / 20, window.getSize().x / 4.8, window.getSize().y / 20).contains(_mouse.x,_mouse.y) && !focus) { 	//Если нажат клаиша над объектом Input...
